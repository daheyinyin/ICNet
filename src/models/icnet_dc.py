...
class ICNetdc(nn.Cell):
    ...
    def __init__(self, nclass=19, pretrained_path="", istraining=True, norm_layer=nn.SyncBatchNorm):
        ...
        self.backbone = SegBaseModel(pretrained_path=pretrained_path, istraining=istraining)
    ...
    def construct(self, x, y):
        ...
        x_sub2, _, _ = self.backbone(x_sub2)

        # sub 4
        _, _, x_sub4 = self.backbone(x)
...
class SegBaseModel(nn.Cell):
    ...
    def __init__(self, nclass=19, backbone='resnet50', pretrained_path=None, istraining=False):
        super(SegBaseModel, self).__init__()
        self.nclass = nclass
        if backbone == 'resnet50':
            self.pretrained = get_resnet50v1b(ckpt_root=pretrained_path, istraining=istraining)
