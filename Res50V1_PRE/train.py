...
if __name__ == '__main__':
    ...
    # init context
    context.set_context(mode=context.PYNATIVE_MODE, device_target=args_opt.device_target, save_graphs=False)
    # GRAPH_MODE
    ...
    if args_opt.is_modelarts == "True":
        ...
        local_zip_path = os.path.join(local_zipfolder_url, str(rank_id), filename)
    ...
    # create dataset
    dataset = create_dataset(dataset_path=local_train_data_url,
                             do_train=True,
                             repeat_num=1,
    ...
    dataset_sink_mode = args_opt.device_target != "CPU"
    model.train(config.epoch_size, dataset, callbacks=cb,
                sink_size=dataset.get_dataset_size(), dataset_sink_mode=dataset_sink_mode)

    if rank_id == 0 and args_opt.is_modelarts == "True":
